local b = require("_rmis_base_builtins")
local pb = b
...
---@param iterable table<any>
---@return table<any>
function pb.map(fn, iterable)
    local res = {}
    local n = table.maxn(iterable)
    for i = 1, n do
...
function pb.sorted(iterable, key, reverse)
    b.not_implemented_error()
end
...
if debug.getinfo(3) == nil then
    print(pb.any({ 1, nil, 3 }))
    print(pb.all({ 1, 3, 3, nil }))
    print(b.list_tostring(pb.filter(function(x)
        return x > 10
